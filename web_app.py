...
- ここに「次へ」ボタンはありません
- **(A) 初めて OAuth 設定する場合**: 手動で **左上の ≡ → API とサービス → OAuth 同意画面** に進む → 次ステップ #4 へ
- **(B) 別プロジェクトで既に OAuth 同意画面を設定済み**の場合: 同意画面をスキップして次の #5 (OAuth クライアント ID 作成) に直行 OK
- どちらも直接 URL を使うのが最速
...
この画面は **4 つのページ** (User Type → アプリ情報 → スコープ → テストユーザー) を順に進みます。1 項目ずつ進めてください。

**4-a. User Type を選ぶ**
1. **User Type** で **「外部 (External)」** を選択
2. 青い **[作成]** ボタンをクリック

**4-b. アプリ情報を入力** (3 項目を 1 つずつ)
1. **アプリ名**: `clip-extractor` と入力 (任意の名前で可)
2. **ユーザーサポートメール**: プルダウンから自分の Gmail を選択
3. **デベロッパーの連絡先情報**: 一番下の欄に自分のメールアドレスを入力
4. **[保存して次へ]** をクリック

**4-c. スコープ**
1. ここでは **何も追加しません** (「スコープを追加または削除」ボタンは押さない)
2. そのまま **[保存して次へ]** をクリック

**4-d. テストユーザー** ★ ここが最重要 ★
1. **[+ ADD USERS]** ボタンをクリック
2. ★ **認証に使う自分の Google アカウント (Gmail) を入力して [追加]** ★
   （この登録を忘れると、後の認証で「アクセスがブロックされました」エラーになります）
3. **[保存して次へ]** をクリック
...
**▼ Phase 3: OAuth 同意画面 (〜3 分、初回のみ)**

12. 🔗 [console.cloud.google.com/apis/credentials/consent](https://console.cloud.google.com/apis/credentials/consent) を開く
    (または左上 ≡ → API とサービス → OAuth 同意画面)
13. **User Type: 外部** を選択 → **[作成]**
    (個人 Gmail だと「内部」は選べないので「外部」だけが表示される)
14. **アプリ情報** 画面:
    - アプリ名: `clip-extractor`
    - ユーザーサポートメール: 自分のアドレス (プルダウンから選択)
    - デベロッパーの連絡先情報: 自分のアドレス
    - 他は空欄 → **[保存して次へ]**
15. **スコープ** 画面: **何も追加せず [保存して次へ]**
    (clip-extractor コード側でスコープを指定するので、ここでの登録不要)
16. **テスト ユーザー** 画面: ★ **最重要** ★
    - **[+ ADD USERS]** / **[+ ユーザーを追加]** をクリック
    - 自分の Google アカウントのメールアドレスを入力 → 追加
    - **[保存して次へ]**
    - ※ これを忘れると後で認証時に「**アクセスがブロックされました**」が出ます
17. **概要** 画面: 入力内容を確認 → **[ダッシュボードに戻る]**

**▼ Phase 4: OAuth クライアント ID 作成 (〜1 分)**

18. 🔗 [console.cloud.google.com/apis/credentials](https://console.cloud.google.com/apis/credentials) を開く
    - ページ上部の **[+ 認証情報を作成]** をクリック
    - プルダウンから **[OAuth クライアント ID]** を選択
      (「API キー」「サービス アカウント」ではない)
...
19. 「**OAuth クライアントを作成しました**」ダイアログが出たら
    ...
    - ※ 閉じてしまっても 認証情報リスト右端の **↓** アイコンからいつでも再取得可
...
1. 🔗 [console.cloud.google.com/auth/audience](https://console.cloud.google.com/auth/audience) を開く
   - 新 UI (Google Auth Platform) の場合: 左サイドバーから **[対象]** をクリック
   - 旧 UI の場合: API とサービス → OAuth 同意画面 → [対象] タブ
...
対処: 認証情報ページ → 該当クライアントを削除 → **「デスクトップ アプリ」** で作り直し → JSON 再ダウンロード
