...
2. 検索欄に `Google Drive API` → カードをクリック → **[有効にする]**
3. 同じく ✓ API が有効です が出たら成功

**3-d. 有効化済み → 次の画面へ** (画面 3 から先への進み方):
- ここに「次へ」ボタンはありません
- 手動で **左上の ≡ → API とサービス → OAuth 同意画面** に進んでください
- もしくは次ステップ (#4) の直接 URL を使うのが最速
...
3. **画面上部のプロジェクトセレクタ** (青いバー内) をクリック
4. ポップアップ右上の **[+ 新しいプロジェクト]** をクリック
5. プロジェクト名に `clip-extractor` (任意) と入力 → **[作成]**
6. 作成完了通知が出たら、上部のセレクタで**作成したプロジェクトに切り替え**
   (切り替え忘れが以降の全操作を無効にする No.1 原因)
