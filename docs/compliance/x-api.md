...
- The four values are stored in the gitignored local `.x_credentials.json` sidecar, matching the existing `.gemini_key` / `.obs_password` pattern. They are excluded from `default_settings.json`.
...
- `.x_credentials.json` is plaintext at rest. The app requests owner-only permissions where POSIX modes are supported, but Windows access is governed by the user's filesystem account and ACLs.
...
- Decide whether plaintext credential storage is acceptable for the supported Windows threat model or replace it with an OS credential store.
- Include the ISC license notice for `requests-oauthlib` and update the release SBOM/NOTICE materials.
- Run secret, dependency, SAST, and final artifact scans and verify that `.x_credentials.json`, logs, test fixtures, and real credentials are absent.
