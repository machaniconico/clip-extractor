...
- [x] For any non-Windows distribution, decide whether the documented plaintext fallback is acceptable or replace it with that platform's credential store. Decided 2026-09-07: support is Windows 10/11 only (`docs/TERMS.md` §1, `docs/PRIVACY.md` §6, `README.md`), so supported installs always use user-scoped DPAPI and the plaintext fallback applies only to unsupported platforms.
...
- [x] Attach the SBOM to a released distributable. `.github/workflows/release.yml` builds `clip-extractor-<tag>.zip` with `git archive` on a `v*` tag, generates the CycloneDX SBOM from the extracted archive's `requirements.lock`, and attaches ZIP, SBOM, and `SHA256SUMS.txt` to a draft GitHub Release (procedure in `docs/RELEASING.md`).
...
- [x] Scan the final packaged artifact before release. The release workflow inspects every ZIP entry against a deny-list of secret, log, output, and tooling paths, verifies CRLF on the batch launchers, then runs `tools/check_secrets.py --directory`, `generate_notices.py --check`, `pip-audit`, and `bandit` on the extracted archive. Any failure aborts before the draft release is created.
...
- [x] Publish user-facing terms and privacy disclosures covering X API posting, credentials, costs, revocation, retention, and support. `docs/TERMS.md` and `docs/PRIVACY.md` are complete for the decided model (individual distributor `machaniconico`, free of charge, GitHub Release only, Japan-resident users aged 18+, Windows 10/11 only, support via GitHub Issues, Japanese law) and linked from `README.md`. Human legal review is still outstanding (below).
- Obtain human legal review for the intended distribution model (individual, free, GitHub Release, Japan only). Points to raise: the liability and venue clauses in `docs/TERMS.md` §8 and §12.
