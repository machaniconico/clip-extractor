...
OBS連携の自動処理は次の二択です。処理中に別の取得元へ自動で切り替えることはありません。

1. `record` — 配信とOBS録画を同時に行い、録画停止後にローカル録画をすぐ処理
2. `stream` — 配信停止後、YouTubeの再エンコード完了を待って正式アーカイブをDLして処理

すぐ切り抜きを作りたい場合は `record`、OBS録画を残さずYouTube上の完成映像を使いたい場合は `stream` を選択してください。
`folder` 監視は `record` 専用です。YouTube概要欄へのタイムスタンプ自動追加は `stream` のみ対応します。
`stream` の待機中はClip Extractorを起動したままにしてください（現在の待機上限は6時間です）。上限を超えた場合は、後日 **Input** タブへ完成アーカイブURLを貼って生成できます。
