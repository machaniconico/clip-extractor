...
配信開始時にXへ告知する場合は、OBS連携タブの **「X 配信開始ポスト」** をONにします。WebSocketで配信開始を検知すると、毎回新しいX投稿作成画面を開き、本文を入力済みにします。最終的なポストは内容を確認して手動で押します。`{links}` には、認証済みYouTubeから取得したその配信のライブURLと、設定欄に1行1件で登録したTwitch・Kickなどの同時配信先URLが入ります。Xの投稿URLやYouTubeのライブURLが配信ごとに変わっても、イベントごとに作り直すため固定保存は不要です。
