...
OBS連携の停止イベントを `stream` にすると、配信終了後の取得元を次の順で自動選択します。

1. 書き込みが完了したOBSローカル録画
2. YouTube視聴画面で再生できる終了直後のpost-live DVR
3. YouTubeの再エンコードが完了した正式アーカイブ

post-live DVRは取得後に実際の動画尺を検証し、不完全な場合は正式アーカイブの処理完了を待って自動的にフォールバックします。
